...
def main(input_bus, input_rev, input_feature, city, state, keywords, k, output):
    ...
    business = spark.read.json(input_bus).repartition(50).select('business_id', 'categories', 'city', 'state', 'stars')
    ...
    results = data.createOrReplaceTempView("recommend")
    ...
    ###
    final_table = bus_rec_fea_join.select(bus_rec_fea_join['address'],bus_rec_fea_join['business_id'].alias('businessId'),bus_rec_fea_join['name'].alias('businessName'),\
